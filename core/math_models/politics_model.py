...
class PoliticsModel(MathModel):
    def calculate_probability(self, market, external_data=None) -> dict:
        ...
        if any(w in q_lower for w in ['incumbent', 'reelection', 're-election',
                                       'current president', 'sitting', 'current governor']):
            incumbency_bonus = 0.03
        ...
        prob = yes_price + (momentum * 0.30) + incumbency_bonus
