...
class HaikuConfirmer:
    ...
    def confirm_edge(self, market, model_result: dict, edge_result) -> HaikuResult:
        ...
        prompt = (
            ...
            f"CONFIRM if:\n"
            f"  - Method is Elo/GBM/momentum/base rate AND edge is 3-30%\n"
            f"  - Method is RandomForest AND edge is 13-50% (RF naturally produces large edges when market is far from model)\n"
            f"DENY only if:\n"
            f"  - Non-RF method AND edge >40% (likely model error)\n"
            f"  - Method is fundamentally wrong for this market type\n"
            f"  - RF AND edge >60% (extreme overfit signal)\n"
            f"A RandomForest edge of 15-45% is normal when the market underprices a contract.\n\n"
